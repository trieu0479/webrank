import api from './reportFunctions.js'; 
...
$(document).ready(() => {
            ...
            if (!originalDomain) {
                ...
                // api("getUniqueUsersMonthly", domain);
                // api("getTrafficAndEngagementAvgVisitDurationMonthly", domain);

                api("getMarketingMixOverviewDaily", domain);
                // // api("getMarketingMixOverviewWeekly",domain);
                // api("getMarketingMixOverviewMonthly",domain);

                api("getTrafficDisplayPaidOutgoingAdsTable", domain);

                api("getTrafficSourcesTotalReferrals", domain);
                ...
                api("getWebDemographicsGender", domain);
                api("getTrafficSourcesOverview", domain);
                api("getTrafficDisplayAdvertisingAds", domain);
                api("getTrafficSourcesSocial", domain);
                api("getTrafficSourcesSearch", domain);
                // api("getGetAudienceInterests", domain);
                api("getWebDemographicsAge", domain);
                api("getSearchOrganicPaidOverview", domain);
                api("getWebsiteAdsIntelDisplay", domain);
                api("getScrapedSearchAds", domain);
                api("getSearchBrandedKeywords", domain);
                //api("getPaidSearchCompetitorsTable", domain);

                api("getTrafficDestinationAds", domain);
                api("getTrafficDisplayPaidOutgoingAds", domain);
    ...
    $('#getTrafficAndEngagement a[data-toggle="tab"]').on('shown.bs.tab', function (e) {
      ...
      if(taskName == "getTrafficAndEngagementVisits")
        taskName = "getTrafficAndEngagement--MonthlyVisits";
      else if(taskName == "getTrafficAndEngagementAvgVisitDuration")
        taskName = "getTrafficAndEngagement--AvgVisitDuration";
      else if(taskName == "getTrafficAndEngagementPagesPerVisit")
        taskName = "getTrafficAndEngagement--PagesPerVisit";
      else if(taskName == "getTrafficAndEngagementBounceRate")
    ...
    initDatatable(
      'getTrafficDisplayAdvertisingWebsitesTable',
      {
        ...
        columns: [
          { title: 'Trang web', data: data => `<div class="d-flex"><img class="p-1 mr-2 border rounded bg-secondary" src="${data.Favicon}"/>${data.Domain} </div>`,width: "30%" },
          { title: 'Lượt truy cập', data: data => (data.Visits==0) ? `-` : `${numeral(data.Visits).format("0,0")}` },
          { title: 'Tỉ lệ truy cập', data: data => `
          <div class="row align-items-center" style="width:150px">
              <div class="col-4">${numeral(data.Share).format('0.00%')}</div>
