...
$(document).ready(() => {     
    ...
    const textdoithu = () => `Bảng phân tích đối thủ cạnh tranh của từ khóa “<strong class="font-gg font-16 text-success">${keywords}</strong>”`;
    ...
    const renderTable = ({
        ...
        if (task == "KeywordSerpAnalysis" ) {
            task="KeywordSerpAnalysis";
            count=5;
        }  
        let tableOptions = {
            ajax: {
            ...
            dom: `<"d-flex no-block pl-4 pr-2 mb-0"<"#${task}--header.font-gg font-16 align-self-center"><"ml-auto abc"B>>lftipr`,
            // dom:'Bftiplr',
            pageLength,
            info: true,
                    ...
                    renderTable({
                        ...
                        dataSrc: res => {
                            ...
                            let {
                              ...
                              console.log(ng);
                              if (!ng) {
                                  $('table#KeywordRecommendationsPhraseMatch').addClas('empty-state');
                              }
            ...
            case "KeywordSerpAnalysis":
                var t = renderTable({
                    ...
                    dataSrc: res => {                               
                        ...
                        // if (res.data == null) {
                        //    $('#KeywordSerpAnalysis').hide()                      
                        // } else if ((res).data.length > 0) {
                        //     let header = texttop10trangSEO();
                        //     $(`#${task}--header`).html(header);
                        //     return ((res).data).filter(item => item.Domain != 'grid.upgrade')
                        ...
                        //     return [];
                        // } 
                        let header = textdoithu();
                        $(`#${task}--header`).html(header);
                        // return ((res).data).filter(item => item.Domain != 'grid.upgrade')              
                     console.log(res.data.data_serp);
                     return res.data.data_serp;
                    },
                    columns: [{
                          
                            data: null,                           
                            className: ' parent-hover align-middle',
                            width: '5px'
                        },
                        {   
                            data: 'url',
                       ...
                       ,
                        {  
                            data: 'content_perform',
                            render:(data,type,row) => 
                              `
                            <div class="row">
                                <div class="col-3 font-14">${data}</div>
                                <div class="col align-self-center">
                                    <div class="progress bg-dark-2" style="height:6px">
                                        <div class="progress-bar bg-info" style="width:${data}%;height:6px;" role="progressbar"></div>
                                    </div>
                                </div>
                            </div>`,                                                
                            className: 'align-middle',
                        ...
                        },
                        {
                            ...
                            data: 'focus_nr_kw',
                            render:(data,type,row) =>`<div class="round round-sm ${data <= 70 ? data <= 30 ? 'round-success' : 'round-warning' : 'round-danger'}">${data ? data : 0}</a> `,                                                
                            className: 'align-middle',
                            width: '40px',
                        },
                        {
 ...
 const ShowSerpAnalys =(task) => {
    $.ajax(`//localapi.trazk.com/keywords/keywords.php?task=KeywordSerpAnalysis&keywords=${keywords}`)
                    .done(function (data) {
                        // console.log(res);
                          let x =data.data.data_header.difficulty;
                          let y =data.data.data_header.kw_difficulty;
                          let z =data.data.data_header.cont_difficulty;
                        ...
                        //  else {                          
                            ...
                            //       console.log('task',task);                                                                      
                                        ...
                                        const eleNumber = (value, format, postfix = '') => $(`<div class="display-6 font-qs counter ml-auto mt-2">${numeral(value).format(format)}${postfix}</div>`);                                       
                                        let eleCPC_1 = eleNumber(x, '0', '');                                       
                                        $(`#KeywordSerpAnalysis--CPC1`)
                                        .append(eleCPC_1)
                                        // .after(`<div class="ml-auto" id="${task}--chartCPC" style="width:50%;height:100%"></div>`)
                                        .parent().removeClass('is-loading')    
                                        let eleCPC_2 = eleNumber(y, '0', '');                                       
                                        $(`#KeywordSerpAnalysis--CPC2`)
                                        .append(eleCPC_2)
                                        // .after(`<div class="ml-auto" id="${task}--chartCPC" style="width:50%;height:100%"></div>`)
                                        .parent().removeClass('is-loading')    
                                        let eleCPC_3 = eleNumber(z, '0', '');                                       
                                        $(`#KeywordSerpAnalysis--CPC3`)
                                        .append(eleCPC_3)
                                        // .after(`<div class="ml-auto" id="${task}--chartCPC" style="width:50%;height:100%"></div>`)
                                        .parent().removeClass('is-loading')    
 ...
 ShowSerpAnalys()
    const doMainRequest = () => {
        ...
        getKeywords(() => {
            ...
            renderChart('KeywordSerpAnalysis')
        });
    }
