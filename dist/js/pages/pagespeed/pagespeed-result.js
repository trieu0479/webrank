...
var websiteUrl = test.searchParams.get("websiteUrl");

if (websiteUrl.indexOf("http") == -1) websiteUrl = `http://${websiteUrl}`

async function getWebsitePageSpeedMobile() {
	let urlMoblie = `//localapi.trazk.com/pagespeed/index.php?task=getWebsitePageSpeedMobile&userToken=${userToken}&url=` + websiteUrl;
...
getWebsitePageSpeedMobile().then(res => {
	...
	scoreMobi = datamobi.lighthouseResult.categories.performance.score;
	SpeedWeb(datamobi)
	logPageSpeedTest();
	
})
async function getWebsitePageSpeed() {
	let urlPc = `//localapi.trazk.com/pagespeed/index.php?task=getWebsitePageSpeed&userToken=${userToken}&url=` + websiteUrl;
	return await $.get(urlPc);
}
getWebsitePageSpeed().then(res => {
	datapc = JSON.parse(res);
	scorePc = datapc.lighthouseResult.categories.performance.score;
	
	
	logPageSpeedTest();
	// SpeedWeb(datapc)
})
var logPageSpeedTest = function logPageSpeedTest(){
	if (logPageSpeedTest.done == true) return;
	if (parseInt(scoreMobi*100) >0 && parseInt(scorePc*100)>0){
		width = 100
		$('.progresss').addClass('display-none');
		$.ajax({
			url: `//localapi.trazk.com/pagespeed/index.php?task=logPageSpeed&userToken=${userToken}&url=${websiteUrl}&mobileScore=${scoreMobi*100}&pcScore=${scorePc*100}`,
			type: "POST"
		}).then(data => {
			data = JSON.parse(data);
			console.log(data);
		});
		logPageSpeedTest.done=true;
	}
}
