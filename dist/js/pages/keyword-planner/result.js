...
$(document).ready(() => {
    ...
    let sortBy = url.searchParams.get("sortBy") ? url.searchParams.get("sortBy") : "relevance";
    ...
    if (keywords == "") {
        ...
        getKeywordsSuggestion(arrKeywords,network);
        logUserSearch();
    ...
    function logUserSearch(){
        ...
        $.post(`//localapi.trazk.com/fff/quangcao.php?task=insertToLogV2&userToken=${userToken}&toolName=keywords`,post)  ;
    }
    ...
    function getKeywordsSuggestion(keywords,network="web"){
        q = keywords[0];
        if (network == "web") network = "";
        if (language == "vn") hl = "vi"; else hl = language;
        console.log(hl);
    ...
    function createError(e) {
        ...
        switch (e) {
            ...
            case "emptyDataKeywordBidforcast":
                Swal.fire('Không dủ dữ liệu để ước tính', `Không đủ dữ liệu ước tính 30 ngày tiếp theo của ${keywords}. Vui lòng nhập từ khóa khác `, 'error');
                break;
            case "emptyDataKeywordSuggesstion":
                Swal.fire('Không có kết quả trả về', `Không tìm thấy gợi ý liên quan đế từ khóa ${keywords}. Vui lòng nhập từ khóa khác `, 'error');
                break;
